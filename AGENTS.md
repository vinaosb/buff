...
**Generated:** 2026-07-21 (regenerated by `/init-deep`; originally v0.1 → v1.0 → v1.9 refresh)
**Commit:** 5723a42 (master, post-T135 SSR/mobile complete — v1.9 in progress)
**Branch:** master (tags: v0.1.0, v0.5.0, v1.0.0, v1.1.0, v1.2.0; v0.1-dev preserved as historical marker)

## OVERVIEW

Buff — high-level language that transpiles `.buff` → Rust → native via rustc/LLVM (and `.buffhtml` SFC → Dioxus 0.7 component → wasm32-unknown-unknown). Implemented as a **19-crate** Rust workspace. Hides borrow-checker pain from users; compiler emits only "easy" Rust. v1.0 "Production" shipped: heterogeneous CPU/GPU computing. v1.1 "Try Buff" shipped: playground + tree-sitter + website. v1.2 "Use Buff" shipped: LSP server + VSCode extension. v1.3-v1.9 in progress: stdlib expansion (T124), REPL (T125a), registry (T126-T127), Jupyter kernel (T129), UI/Dioxus (T130-T131), RSX-for-Buff `.buffhtml` (T133-T135). Next: v1.13-v1.17 frameworks roadmap (see `.sisyphus/plans/buff-v1x-frameworks.md`).

## STRUCTURE

```
buff/
├── crates/                              # 19 workspace members (the compiler + tooling)
...
│   ├── buff-lang-codegen-rust/          # AST → syn::File → prettyplease → Rust source (+ race/atomic/gpu_alignment analyses + T124 stdlib lowering)
...
- **CI clippy DOES include `--all-targets`** (CI line 18). CONTRIBUTING.md note saying otherwise is OUTDATED. README.md "Building from source" omits `--all-targets` (cosmetic).
...
- **Two version tiers**: 1.2.0 (10 core compiler crates: ast/lexer/parser/types/codegen-rust/codegen-wgsl/runtime/error/cli/playground-wasm/lsp) and 1.0.0 (9 newer tooling crates). Bump these explicitly when cutting a release.
