...
- [x] **T133: RSX-for-Buff parser/lexer extension** [ultrabrain]
  ...
  **Acceptance Criteria** (re-mapped under Option C pivot; original criteria assumed compiler changes that did NOT happen):
  ...
  - **Stretch features SHIPPED at `480b638`** (T133 backfill, 111 tests +30 net): named slots (`<slot name="x" />`), keyed each (`{#each xs as x (x.id)}` — also fixed the `(`-in-iterable limitation via `find_each_as_boundary` with paren/bracket/string depth tracking), spread props (`{...rest}` → `..rest`), two-way binding (`bind:value={name}` → controlled form `value: name, oninput: move |__e| name.set(__e.value())`), await blocks (`{#await fut}{:then b}{:catch e}{/await}` → `use_resource` hook + `ResourceState::Ready` match arms), `{@html}` escape hatch (→ `div { dangerous_inner_html: <expr>, data_buffhtml_xss: "{@html} opt-in" }` — audit marker as runtime HTML attribute since prettyplease strips `//` comments inside macro bodies).
  - **Still deferred to T134+**: full `.buffhtml` LSP, tree-sitter grammar, VSCode extension beyond TextMate, prop type pre-checker, cross-file LSP features, full Buff-syntax script-block transpilation (`component`/`state`/`fn` shorthand — T133 ships Rust-in-script-block pass-through).
