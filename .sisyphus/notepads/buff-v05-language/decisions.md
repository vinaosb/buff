...
(returns `None` — an extern-crate decl has no item name to export).
...
- Parser: `parse_func_decl` consumes an optional leading `extern`
  ...
  is parsed — an empty placeholder `Block` is synthesised.
- Codegen: `Decl::FuncDecl where is_extern` routes to
  `lower_extern_func_decl` → `syn::ItemForeignMod` (bodyless
...
codegen simple (each decl → one item) at the cost of slightly more
...
### Decision: codegen-level dep-set collection; CLI→Cargo.toml DEFERRED
...
`use serde;` emission + documented deferral), NOT via real CLI→Cargo.toml
...
The task spec §3 explicitly allows this fallback: "If full CLI wiring
...
warnings — callers should qualify paths explicitly
(e.g. `serde::Serialize`). Crate names with `-` are normalised to `_`
(`rust-decimal` → `use rust_decimal;`) matching crates.io convention.
...
independent of hash seed — the T29 flaky-test lesson ("never rely on
...
- "Configurable" (per the task's REFACTOR ask) is satisfied by having a
  ...
  (Rust→Buff for diagnostics) or override mechanism can consult.
