...
- T133 (52e59aa+1d879f3+dad8578): Chose Option C (.buffhtml SFC) over Options A (embedded JSX) and B (indent DSL) per Oracle design spike e1a5e74. A would force Pratt-parser rewrite for < / > disambiguation (the <T,> generic-arrow TS pain is structural). B overloads : a 6th time. C confines pioneer-risk to a new FILE FORMAT (Vue/Svelte precedent) rather than the LANGUAGE (first-of-kind). All compiler work additive; zero changes to existing lexer/parser/AST/codegen-rust. Span-mapping spike VERDICT PASS confirmed rustc errors inside rsx!{} are token-precise via dioxus-rsx quote_spanned! — post-format text scan suffices, no Location::caller() plumbing. Script-block = Rust-pass-through for T133 floor; full Buff transpilation deferred to T134.
- T133 v1.9 floor intentionally narrow per decision record section 6: ships grammar + parser + codegen + CLI integration + TextMate + 2 examples + e2e recipe. DEFERS to T134+: named slots, keyed each, spread props, two-way binding, await, {@html}, full LSP, tree-sitter grammar, prop type pre-checker, cross-file LSP, full Buff-syntax script-block. This is the documented scope discipline to ship v1.9 without gold-plating.
