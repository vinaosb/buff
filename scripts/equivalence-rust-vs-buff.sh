...
for test_def in "${TESTS[@]}"; do
    ...
    buff_output=$($BUFF run "$buff_file" 2>&1 | grep -v "^info:" | grep -v "^$" || true)

    # Run Rust example — filter to ONLY numeric/boolean lines (the actual program output)
    rust_output=$(cargo run -p "$crate" --example "$example" --release 2>&1 | grep -E '^[0-9]+$|^true$|^false$' || true)
