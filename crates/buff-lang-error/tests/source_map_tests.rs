//! Integration tests for the Buff ↔ Rust bidirectional source-map mapping (T16).
//!
//! These exercise [`SourceMap::add_mapping`], [`SourceMap::lookup_buff`], and
//! [`SourceMap::lookup_rust`] — the back-end line-mapping API that lets
//! `buff_lang_cli::error_mapper` translate `rustc`/runtime locations back to the
//! original `.buff` source.
//!
...
fn test_source_map_round_trip() {
    ...
    // Forward: rust line → buff span.
    let looked_up = sm.lookup_buff(15);
    assert_eq!(
        looked_up,
        Some(span),
...
fn test_source_map_multiple_mappings_round_trip() {
    ...
    assert_eq!(sm.lookup_buff(3), Some(span_a));
    assert_eq!(sm.lookup_buff(7), Some(span_b));
    assert_eq!(sm.lookup_buff(12), Some(span_c));
...
fn test_source_map_no_mapping_returns_none() {
    ...
    assert_eq!(sm.lookup_buff(1), None);
    assert_eq!(sm.lookup_buff(100), None);
    assert_eq!(sm.lookup_rust(make_span(0, 10)), None);
}

#[test]
fn test_source_map_lookup_buff_below_all_mappings_returns_none() {
    let mut sm = SourceMap::new();
    sm.add_mapping(make_span(0, 5), 10);
    sm.add_mapping(make_span(10, 15), 20);

    // Line 5 is below both mappings (10 and 20).
    assert_eq!(sm.lookup_buff(5), None);
}
...
fn test_source_map_closest_below() {
    ...
    assert_eq!(sm.lookup_buff(10), Some(span_10));
}
...
fn test_source_map_closest_below_with_gap() {
    ...
    assert_eq!(sm.lookup_buff(150), Some(span));
    // Line 99 is below 100 — no mapping.
    assert_eq!(sm.lookup_buff(99), None);
}
...
fn test_source_map_front_end_still_works() {
    ...
    assert_eq!(sm.lookup_buff(2), Some(make_span(6, 11)));
}
