//! Integration tests for `deox-types` — literal inference, promotion,
//! operator typing, identifier lookup, control-flow typing, and error cases.

use deox_ast::{BinaryOp, Block, Expr, Ident, Literal, Stmt, TypeRef, UnaryOp};
use deox_error::{Diagnostic, Severity, Span};
use deox_types::{promote_binary, FloatWidth, IntWidth, Type, TypeInferencer};
...
fn test_type_error_carries_span() {
    let mut inf = TypeInferencer::new();
    let span = Span::new(10, 20, deox_error::SourceId(7));
