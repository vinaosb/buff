...
fn main() {
    ...
    for step in 1..=3 {
        world.tick();
        let pos = world.get_clone::<Position>(player).unwrap_or(Position { x: -1.0, y: -1.0 });
        println!("after tick {step}: Position {{ x: {}, y: {} }}", pos.x, pos.y);
    }

    let final_pos = world.get_clone::<Position>(player).unwrap_or(Position { x: -1.0, y: -1.0 });
