...
//! intermediate `.rs` file back to the original `.deox` source location.
//!
//! ## v0.1 strategy
//!
//! Exact Deox line translation requires a populated
//! [`SourceMap`](deox_error::SourceMap) (Deox span ↔ Rust line), which in turn
...
//!   original `.deox` path in `rustc`'s stderr.
...
//!   `.deox` path. If a [`SourceMap`](deox_error::SourceMap) with line mappings
//!   is available, the Rust line number is also translated to the closest Deox
...
//!   frames originating from their Deox program.

use std::path::Path;

use deox_error::SourceMap;
...
/// points at the user's `.deox` source instead.
///
/// Both forward-slash and backslash variants are replaced so the function
/// works on both Unix and Windows `rustc` output.
pub fn translate_rustc_errors(stderr: &str, deox_file: &Path, rust_file: &Path) -> String {
    let rust_str = rust_file.to_string_lossy();
    let deox_str = deox_file.to_string_lossy();
    // Replace both the native path and any slash-variant rustc may emit.
    let mut result = stderr.replace(rust_str.as_ref(), deox_str.as_ref());
    // Also handle the case where rustc normalizes to forward slashes.
    let rust_fwd = rust_str.replace('\\', "/");
    let deox_fwd = deox_str.replace('\\', "/");
    if rust_fwd != rust_str.as_ref() {
        result = result.replace(rust_fwd.as_str(), deox_fwd.as_str());
    }
    result
}

/// Translate a runtime panic message, replacing the `.rs` file reference with
/// the `.deox` file reference.
///
/// If `source_map` contains Rust-line → Deox-span mappings, the Rust line
/// number in the panic location is additionally translated to the closest Deox
...
/// becomes:
/// ```text
/// thread 'main' panicked at 'attempt to divide by zero', prog.deox:2:15
/// ```
pub fn translate_panic(
    ...
    let rust_str = rust_file.to_string_lossy();
    let deox_str = deox_file.to_string_lossy();
    let mut result = panic_msg.replace(rust_str.as_ref(), deox_str.as_ref());
    ...
    // We look for the pattern "<deox_file>:LINE:COL" that we just produced and
    // replace LINE with the best Deox line. This is best-effort.
    if !source_map.is_line_map_empty() {
        result = translate_panic_line_numbers(&result, deox_file, source_map);
    }

    result
}

/// Given a panic string that already has the `.deox` filename substituted in,
/// find `<deox_file>:RUST_LINE:COL` patterns and replace `RUST_LINE` with the
/// closest Deox line from the source map.
fn translate_panic_line_numbers(msg: &str, deox_file: &Path, source_map: &SourceMap) -> String {
    let deox_str = deox_file.to_string_lossy();
    let mut result = String::with_capacity(msg.len());
    let mut rest = msg;

    while let Some(pos) = rest.find(deox_str.as_ref()) {
        // Copy everything up to and including the filename.
        result.push_str(&rest[..pos + deox_str.len()]);
        rest = &rest[pos + deox_str.len()..];
        ...
        if let Some(colon_pos) = rest.find(':') {
            ...
            if digit_end > 0 {
                ...
                if let Ok(rust_line) = line_str.parse::<usize>() {
                    if let Some(deox_span) = source_map.lookup_deox(rust_line) {
                        // Look up the Deox line from the span's source_id.
                        if let Some((deox_line, _)) =
                            source_map.lookup(deox_span.source_id, deox_span.start)
                        {
                            result.push(':');
                            result.push_str(&deox_line.to_string());
...
/// which are noise from the Deox user's perspective. The patterns filtered:
///
...
mod tests {
    ...
    fn deox_path() -> PathBuf {
        PathBuf::from("/tmp/prog.deox")
    }

    #[test]
    fn translate_rustc_errors_replaces_path() {
        let stderr = "error[E0384]: cannot assign\n  --> /tmp/prog.rs:15:5\n   |\n15 |     x = 2;\n   |     ^^^^^";
        let result = translate_rustc_errors(stderr, &deox_path(), &rust_path());
        assert!(
            result.contains("prog.deox:15:5"),
            "expected deox path in result, got: {result}"
        );
        assert!(
    ...
    fn translate_panic_replaces_path_empty_map() {
        ...
        let result = translate_panic(panic, &rust_path(), &deox_path(), &sm);
        assert!(
            result.contains("prog.deox:2:15"),
            "expected deox path + original line, got: {result}"
        );
        assert!(
            !result.contains("prog.rs:"),
            "should not contain .rs path, got: {result}"
    ...
    fn translate_panic_translates_line_with_map() {
        // Build a source map: rust line 5 → deox span at byte 10 (line 2).
        let mut sm = SourceMap::new();
        let sid = deox_error::SourceId(0);
        sm.add_source(
            sid,
            PathBuf::from("prog.deox"),
            "line1\nline2\nline3\nline4\nline5\n".to_string(),
        );
        // Byte offset 6 = start of "line2" = line 2.
        let deox_span = deox_error::Span::new(6, 11, sid);
        sm.add_mapping(deox_span, 5);

        let panic = "thread 'main' panicked at 'boom', prog.rs:5:10";
        let result = translate_panic(panic, Path::new("prog.rs"), Path::new("prog.deox"), &sm);
        // The rust line 5 should map to deox line 2.
        assert!(
            result.contains("prog.deox:2:"),
