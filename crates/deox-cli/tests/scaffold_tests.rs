//! Integration tests for `deox new` / `deox init` project scaffolding.
//!
...
/// `deox new` tests running concurrently would each create their project under
...
/// Per-test working directory root: `<temp>/deox-scaffold-tests-<pid>/`.
fn temp_root() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("deox-scaffold-tests-{}", std::process::id()));
...
// ---------------------------------------------------------------------------
// `deox new` — filesystem scaffolding
// ---------------------------------------------------------------------------
...
fn test_new_creates_project_with_all_files() {
    ...
    // `deox new` writes into the *current* working directory; chdir into the
    ...
    result.expect("`deox new` should succeed for a valid name");

    let project_dir = workdir.join(project_name);
    for rel in &["deox.toml", "src/main.deox", ".gitignore", "README.md"] {
        let p = project_dir.join(rel);
        assert!(
            p.exists(),
            "expected `{}` to exist after `deox new`",
            p.display()
        );
    }

    // Content sanity-check: deox.toml should reference the project name.
    let toml = fs::read_to_string(project_dir.join("deox.toml")).unwrap_or_default();
    assert!(
        toml.contains("name = \"test_app\""),
        "deox.toml should embed the project name; got:\n{toml}"
    );

    // main.deox must contain a valid `func main():` so it can be run.
    let main_deox = fs::read_to_string(project_dir.join("src/main.deox")).unwrap_or_default();
    assert!(
        main_deox.contains("func main():"),
        "main.deox should declare `func main():`; got:\n{main_deox}"
    );
    assert!(
        main_deox.contains("print("),
        "main.deox should call `print`; got:\n{main_deox}"
    );
...
fn test_new_rejects_invalid_name() {
    ...
    assert!(
        ...
        "invalid-name `deox new` should not create a directory"
    );
...
// ---------------------------------------------------------------------------
// `deox init` — current directory scaffolding
// ---------------------------------------------------------------------------
...
fn test_init_scaffolds_current_directory() {
    ...
    result.expect("`deox init` should succeed in a clean dir");

    for rel in &["deox.toml", "src/main.deox", ".gitignore", "README.md"] {
        let p = init_dir.join(rel);
        assert!(
            p.exists(),
            "expected `{}` to exist after `deox init`",
            p.display()
        );
    }

    // The manifest should pick up the directory name.
    let toml = fs::read_to_string(init_dir.join("deox.toml")).unwrap_or_default();
    assert!(
...
fn test_init_refuses_existing_manifest() {
    ...
    let _ = fs::create_dir_all(&init_dir);
    fs::write(init_dir.join("deox.toml"), "[package]\nname = \"old\"\n")
    ...
    assert!(
        msg.contains("already exists"),
        "expected already-exists error for existing deox.toml, got: {msg}"
    );
...
fn test_new_generated_project_runs() {
    ...
    let main_path = workdir.join(project_name).join("src/main.deox");
    std::env::set_current_dir(&original).expect("restore cwd");
    drop(_guard);

    // Compile + run the scaffolded program, capturing stdout.
    // Using commands::run::run forwards stdout to the test process, which
    // doesn't let us assert — so spawn the `deox` lib pipeline via a captured
    ...
    result.expect("generated `deox new` project should run end-to-end");
}
