...
//!
//! These exercise the public functions in `deox_cli::error_mapper`:
//!
//! - [`translate_rustc_errors`] — replaces `.rs` paths with `.deox` paths in
//!   rustc's stderr output.
//! - [`translate_panic`] — replaces `.rs` paths with `.deox` paths in runtime
//!   panic messages.
//! - [`filter_backtrace`] — removes Rust-stdlib frames from a backtrace.
//!
//! Plus one end-to-end test that runs a panicking Deox program via the `deox`
//! binary and verifies the error references the `.deox` file, not the `.rs`.

use std::fs;
use std::path::{Path, PathBuf};
use std::process::Command;

use deox_cli::error_mapper;
...
fn temp_root() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("deox-cli-errmap-tests-{}", std::process::id()));
...
fn test_translate_rustc_errors_replaces_path() {
    ...
    let deox_file = PathBuf::from("/tmp/prog.deox");
    let stderr = "error[E0384]: cannot assign twice to immutable variable `x`\n  --> /tmp/prog.rs:15:5\n   |\n15 |     x = 2;\n   |     ^^^^^";

    let result = error_mapper::translate_rustc_errors(stderr, &deox_file, &rust_file);

    assert!(
        result.contains("prog.deox:15:5"),
        "expected .deox path in translated output, got: {result}"
    );
    assert!(
...
fn test_translate_rustc_errors_preserves_non_path_content() {
    ...
    let deox_file = PathBuf::from("src/main.deox");
    ...
    assert!(result.contains("main.deox"), "file path translated");
}
...
fn test_translate_panic_replaces_path() {
    let rust_file = PathBuf::from("/tmp/prog.rs");
    let deox_file = PathBuf::from("/tmp/prog.deox");
    let sm = deox_error::SourceMap::new();

    let panic_msg =
        "thread 'main' panicked at 'attempt to divide by zero', /tmp/prog.rs:2:15\nnote: run";

    let result = error_mapper::translate_panic(panic_msg, &rust_file, &deox_file, &sm);

    assert!(
        result.contains("prog.deox:2:15"),
        "expected .deox path + preserved line in result, got: {result}"
    );
    assert!(
        !result.contains("prog.rs:"),
...
fn test_translate_panic_preserves_message_content() {
    ...
    let deox_file = PathBuf::from("in.deox");
    let sm = deox_error::SourceMap::new();
    ...
    assert!(result.contains("in.deox"), "file translated: {result}");
}

#[test]
fn test_translate_panic_with_line_map_translates_line() {
    // Build a source map with a mapping: rust line 5 → deox span at byte 6 (line 2).
    let mut sm = deox_error::SourceMap::new();
    ...
    let deox_file = PathBuf::from("prog.deox");
    let panic_msg = "thread 'main' panicked at 'boom', prog.rs:5:10";

    let result = error_mapper::translate_panic(panic_msg, &rust_file, &deox_file, &sm);

    // Rust line 5 should be translated to deox line 2.
    assert!(
...
// ---------------------------------------------------------------------------
// End-to-end: runtime panic translated to .deox
// ---------------------------------------------------------------------------

#[test]
fn test_end_to_end_runtime_error_mapped_to_deox() {
    if !rustc_available() {
        eprintln!("skipping test_end_to_end_runtime_error_mapped_to_deox: rustc not on PATH");
        return;
    }

    // CARGO_BIN_EXE_deox is set by Cargo for integration tests in this crate.
    let deox_bin = std::env::var("CARGO_BIN_EXE_deox")
        .expect("CARGO_BIN_EXE_deox must be set for integration tests");

    // Division by zero panics at runtime in the compiled Rust binary.
    let src = "func main():\n    print(1 / 0)\n";
    let file = write_fixture("runtime_panic.deox", src);

    let output = Command::new(&deox_bin)
        .arg("run")
        .arg(&file)
        .output()
        .expect("failed to spawn `deox run`");

    let stderr = String::from_utf8_lossy(&output.stderr);

    // The translated error should reference the .deox file.
    assert!(
        stderr.contains("runtime_panic.deox"),
        "expected stderr to reference .deox file after translation, got:\n{stderr}"
    );
