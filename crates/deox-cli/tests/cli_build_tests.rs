//! Integration tests for the `deox build` command and the front-end pipeline
//! (`deox_cli::pipeline::compile_to_rust`).
//!
...
fn temp_root() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("deox-cli-build-tests-{}", std::process::id()));
...
fn test_compile_nonexistent_file_returns_clear_error() {
    let bogus = temp_root().join("definitely-does-not-exist.deox");
...
fn test_compile_invalid_syntax_returns_parse_error() {
    ...
    let src = "let x = 5\n";
    let file = write_fixture("invalid_syntax.deox", src);
...
fn test_compile_unterminated_string_returns_lex_error() {
    let src = "func main():\n    print(\"unterminated)\n";
    let file = write_fixture("unterminated.deox", src);
...
fn test_compile_empty_file_succeeds() {
    ...
    let file = write_fixture("empty.deox", "");

    let out = pipeline::compile_to_rust(&file).expect("empty .deox should compile");
    assert!(
    ...
    assert_eq!(
        ...
        ".rs file should sit alongside .deox"
    );
    assert!(
...
fn test_compile_generates_fn_main_for_ola() {
    let src = "func main():\n    print(\"Olá, Deox!\")\n";
    let file = write_fixture("ola_fixture.deox", src);
    ...
    assert!(
        out.rust_source.contains("Olá, Deox!"),
...
fn test_compile_writes_rust_file_alongside_source() {
    let src = "func main():\n    print(42)\n";
    let file = write_fixture("writes_rs.deox", src);
...
fn test_build_command_creates_executable_end_to_end() {
    ...
    let src = "func main():\n    print(\"build e2e\")\n";
    let file = write_fixture("build_e2e.deox", src);
...
fn test_build_command_with_explicit_output_path() {
    ...
    let src = "func main():\n    print(\"explicit\")\n";
    let file = write_fixture("explicit_out.deox", src);
    let rs_path = file.with_extension("rs");
