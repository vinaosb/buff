...
use deox_error::{
    CodegenError, DeoxError, Diagnostic, LexError, ParseError, RuntimeError, Severity, SourceId,
...
fn lookup_out_of_bounds_returns_none() {
    let mut sm = SourceMap::new();
    sm.add_source(SourceId(1), PathBuf::from("test.deox"), "hi".into());
...
#[test]
fn deox_error_from_lex_error() {
    let diag = Diagnostic::error("unknown token", Span::dummy());
    let lex_err = LexError::new(diag);
    let err: DeoxError = lex_err.into();
    assert!(matches!(err, DeoxError::Lex(_)));
}

#[test]
fn deox_error_from_parse_error() {
    let diag = Diagnostic::error("expected semicolon", Span::dummy());
    let parse_err = ParseError::new(diag);
    let err: DeoxError = parse_err.into();
    assert!(matches!(err, DeoxError::Parse(_)));
}

#[test]
fn deox_error_from_type_error() {
    let diag = Diagnostic::error("type mismatch", Span::dummy());
    let type_err = TypeError::new(diag);
    let err: DeoxError = type_err.into();
    assert!(matches!(err, DeoxError::Type(_)));
}

#[test]
fn deox_error_from_codegen_error() {
    let diag = Diagnostic::error("codegen failed", Span::dummy());
    let cg_err = CodegenError::new(diag);
    let err: DeoxError = cg_err.into();
    assert!(matches!(err, DeoxError::Codegen(_)));
}

#[test]
fn deox_error_from_runtime_error() {
    let diag = Diagnostic::error("runtime panic", Span::dummy());
    let rt_err = RuntimeError::new(diag);
    let err: DeoxError = rt_err.into();
    assert!(matches!(err, DeoxError::Runtime(_)));
}

#[test]
fn deox_error_display() {
