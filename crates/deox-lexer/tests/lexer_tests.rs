//! Integration tests for the `deox-lexer` crate's `tokenize()` function.
//!
...
use deox_lexer::{tokenize, Token, TokenKind};
...
fn err(src: &str) -> deox_lexer::LexerError {
    tokenize(src, SourceId(0))
        .err()
...
fn test_tokenize_hello_world() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            TokenKind::StringPart("Olá, Deox!".into()),
            TokenKind::StringEnd,
            TokenKind::RParen,
            TokenKind::Dedent,
            TokenKind::Eof,
...
#[test]
fn test_snapshot_ola_deox() {
    let src = include_str!("../../../tests/fixtures/valid/ola.deox");
    let tokens = tokenize(src, SourceId(0)).expect("ola.deox should tokenize cleanly");
    let lines: Vec<String> = tokens.iter().map(|t| t.to_string()).collect();
    insta::assert_snapshot!(lines.join("\n"));
}

#[test]
fn test_snapshot_arithmetic_deox() {
    let src = include_str!("../../../tests/fixtures/valid/arithmetic.deox");
    let tokens = tokenize(src, SourceId(0)).expect("arithmetic.deox should tokenize cleanly");
    let lines: Vec<String> = tokens.iter().map(|t| t.to_string()).collect();
    insta::assert_snapshot!(lines.join("\n"));
}
