//! Token definition tests for the deox-lexer crate.
//!
...
use deox_error::Span;
use deox_lexer::{LexerError, Token, TokenKind};
...
#[test]
fn lexer_error_into_deox_error() {
    let span = Span::dummy();
    let lex_err = LexerError::new("test", span);
    let deox_err: deox_error::DeoxError = lex_err.into();
    assert!(deox_err.to_string().contains("test"));
}
