...
fn main() {
    let root =
        std::env::temp_dir().join(format!("buff_archive_tar_example-{}", std::process::id(),));
    ...
    let recovered = std::fs::read_to_string(extracted.join("a.txt")).expect("read extracted a.txt");
