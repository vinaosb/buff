...
//!
//! Every binding in Buff is MOVED by default (Rust move semantics). The
...
//!
//! These tests build small Buff ASTs by hand, lower them with
//! [`buff_lang_codegen_rust::generate_rust`], and assert properties of the
...
fn test_generated_rust_compiles_with_rustc() {
    ...
    let rs_path = temp_dir.join("buff_t33a_compile_test.rs");
    let out_path = temp_dir.join("buff_t33a_compile_test.out");
