...
//! - End-to-end: parsing the `ola.buff` and `arithmetic.buff` fixtures

#![allow(clippy::approx_constant)]

use buff_lang_ast::{Decl, Expr, FuncDecl, Stmt};
use buff_lang_error::SourceId;
use buff_lang_lexer::tokenize;
use buff_lang_parser::{parse, parse_func_decl, parse_statement, TokenStream};
...
fn test_func_body_via_indent_dedent_returns() {
    ...
    assert_eq!(
        ...
        Some(format!(
            "{:?}",
            buff_lang_lexer::TokenKind::Ident("print".into())
        )),
...
fn test_dedent_ends_block() {
    ...
    assert!(matches!(
        stream.peek_kind(),
        Some(buff_lang_lexer::TokenKind::Ident(name)) if name == "b"
    ));
}
...
#[test]
fn test_end_to_end_ola_buff() {
    let src = include_str!("../../../tests/fixtures/valid/ola.buff");
    let tokens = tokenize(src, sid()).expect("ola.buff should tokenize cleanly");
    let decls = parse(&tokens, sid()).expect("ola.buff should parse cleanly");
    assert_eq!(decls.len(), 1, "ola.buff defines one function (main)");
    let f = match &decls[0] {
        Decl::FuncDecl(f) => f,
        other => panic!("expected FuncDecl, got {other:?}"),
    };
...
#[test]
fn test_end_to_end_arithmetic_buff() {
    let src = include_str!("../../../tests/fixtures/valid/arithmetic.buff");
    let tokens = tokenize(src, sid()).expect("arithmetic.buff should tokenize cleanly");
    let decls = parse(&tokens, sid()).expect("arithmetic.buff should parse cleanly");
...
fn test_layout_block_preserves_statement_order() {
    ...
    let display = buff_lang_ast::Block::empty(buff_lang_error::Span::dummy()).to_string();
