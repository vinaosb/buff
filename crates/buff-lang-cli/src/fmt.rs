...
use buff_lang_ast::{
    ...
    ReexportDecl, Stmt, StructDecl, TraitDecl, TypeRef,
};
...
/// pre-parsed AST without re-running the parser.
...
impl Formatter {
    ...
    fn write_block_body(&mut self, block: &Block) {
        if block.stmts.is_empty() {
            // No valid `pass`-like token in Buff; emit nothing. Round-trip
            // is best-effort only — real source always has ≥1 statement.
            return;
        }
        self.indent();
