...
/// T1: `src/main.buff` entry-point template that imports from the sibling
/// `lib.buff` (paired with [`LIB_BUFF_TEMPLATE`]). Lets `buff run src/main.buff`
/// and `buff build` (project mode) work end-to-end on a freshly-scaffolded
/// `--lib` project — exercises the multi-file linking pipeline.
pub const LIB_MAIN_BUFF_TEMPLATE: &str = r#"// {name} - entry point (multi-file --lib scaffold, T1)
// Imports the exported symbols from ./lib.buff and exercises them in main.

import { greet, add } from "./lib.buff"
...
/// T1: `src/lib.buff` library module paired with the server template
/// (multi-file --server scaffold). Exports the async handler so the
/// entry-point `main.buff` can import + spawn it.
pub const SERVER_LIB_BUFF_TEMPLATE: &str = r#"// {name} - server library module (multi-file --server scaffold, T1)
// Exports the async handler consumed by ./main.buff.

export async func handler():
    return "Hello from {name}!"
"#;

/// T1: `src/main.buff` entry-point for the multi-file --server scaffold.
/// Imports `handler` from `./lib.buff` and spawns it.
pub const SERVER_MULTI_MAIN_BUFF_TEMPLATE: &str = r#"// {name} - entry point (multi-file --server scaffold, T1)
// Imports the async handler from ./lib.buff and spawns it.

import { handler } from "./lib.buff"
...
/// T1: `src/lib.buff` GPU-dispatch library module paired with the
/// multi-file --gpu scaffold. Exports the `@prefer(gpu)` compute fn
/// so the entry-point `main.buff` can import + call it.
pub const GPU_LIB_BUFF_TEMPLATE: &str = r#"// {name} - GPU library module (multi-file --gpu scaffold, T1)
// Exports the @prefer(gpu) compute fn consumed by ./main.buff.

@prefer(gpu)
export func compute(a, b):
    return a * b
"#;

/// T1: `src/main.buff` entry-point for the multi-file --gpu scaffold.
/// Imports `compute` from `./lib.buff` and exercises it.
pub const GPU_MULTI_MAIN_BUFF_TEMPLATE: &str = r#"// {name} - entry point (multi-file --gpu scaffold, T1)
// Imports the @prefer(gpu) compute fn from ./lib.buff and exercises it.

import { compute } from "./lib.buff"
...
pub fn files_for_template(template: TemplateKind, name: &str) -> Vec<ScaffoldFile> {
    ...
    match template {
        ...
        TemplateKind::Lib => {
            // T1: multi-file --lib scaffold. lib.buff exports, main.buff
            // imports + exercises the exports via `func main()` so
            // `buff build` / `buff run src/main.buff` work end-to-end
            // on a freshly-scaffolded project.
            files.push(("src/lib.buff", render(LIB_BUFF_TEMPLATE)));
            files.push(("src/main.buff", render(LIB_MAIN_BUFF_TEMPLATE)));
        }
        TemplateKind::Server => {
            // T1: multi-file --server scaffold. lib.buff exports the
            // async handler; main.buff imports + spawns it.
            files.push(("src/lib.buff", render(SERVER_LIB_BUFF_TEMPLATE)));
            files.push(("src/main.buff", render(SERVER_MULTI_MAIN_BUFF_TEMPLATE)));
        }
        TemplateKind::Gpu => {
            // T1: multi-file --gpu scaffold. lib.buff exports the
            // @prefer(gpu) compute fn; main.buff imports + calls it.
            files.push(("src/lib.buff", render(GPU_LIB_BUFF_TEMPLATE)));
            files.push(("src/main.buff", render(GPU_MULTI_MAIN_BUFF_TEMPLATE)));
        }
        TemplateKind::Workspace => {
            files.push((
...
mod tests {
    ...
    #[test]
    fn files_for_lib_now_has_main_for_t1_multifile() {
        // T1: --lib scaffold now emits BOTH lib.buff + main.buff so the
        // project compiles + runs end-to-end via buff build / buff run
        // (multi-file linking). The main.buff imports from lib.buff.
        let files = files_for_template(TemplateKind::Lib, "demo");
        let paths: Vec<_> = files.iter().map(|(p, _)| *p).collect();
        assert!(paths.contains(&"src/lib.buff"));
        assert!(paths.contains(&"src/main.buff"));
        // The emitted main.buff MUST import from lib.buff (else it's
        // not actually multi-file).
        let main_buff = files
            .iter()
            .find(|(p, _)| *p == "src/main.buff")
            .map(|(_, c)| c.as_str())
            .expect("main.buff present");
        assert!(
            main_buff.contains("import { greet, add } from \"./lib.buff\""),
            "main.buff must import from lib.buff; got:\n{main_buff}"
        );
    }

    #[test]
    fn files_for_server_is_multifile_t1() {
        // T1: --server scaffold now emits lib.buff + main.buff.
        let files = files_for_template(TemplateKind::Server, "demo");
        let paths: Vec<_> = files.iter().map(|(p, _)| *p).collect();
        assert!(paths.contains(&"src/lib.buff"));
        assert!(paths.contains(&"src/main.buff"));
        let main_buff = files
            ...
            .expect("main.buff present");
        assert!(
            main_buff.contains("import { handler } from \"./lib.buff\""),
            "server main.buff must import from lib.buff; got:\n{main_buff}"
        );
    }

    #[test]
    fn files_for_gpu_is_multifile_t1() {
        // T1: --gpu scaffold now emits lib.buff + main.buff.
        let files = files_for_template(TemplateKind::Gpu, "demo");
        let paths: Vec<_> = files.iter().map(|(p, _)| *p).collect();
        assert!(paths.contains(&"src/lib.buff"));
        assert!(paths.contains(&"src/main.buff"));
        let main_buff = files
            ...
            .expect("main.buff present");
        assert!(
            main_buff.contains("import { compute } from \"./lib.buff\""),
            "gpu main.buff must import from lib.buff; got:\n{main_buff}"
        );
    }
