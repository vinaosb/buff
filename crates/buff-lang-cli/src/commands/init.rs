...
//! run if `buff.toml` already exists (avoids wiping an existing project).
...
/// Entry point for `buff init`.
///
/// Derives the project name from the current directory, validates it, refuses
/// to overwrite an existing `buff.toml`, then writes the scaffold files.
///
/// # Errors
///
/// - The current directory name is not a valid Buff identifier (clear message
///   from [`scaffold::validate_project_name`]).
/// - `buff.toml` already exists in the current directory.
...
pub fn run() -> Result<()> {
    ...
    if manifest_path.exists() {
        bail!("`buff.toml` already exists in the current directory — refusing to overwrite");
    }
    ...
    fs::write(
        &manifest_path,
        scaffold::render_template(scaffold::BUFF_TOML_TEMPLATE, dir_name),
    )
    .with_context(|| format!("failed to write `{}`", manifest_path.display()))?;

    fs::write(
        src_dir.join("main.buff"),
        scaffold::render_template(scaffold::MAIN_BUFF_TEMPLATE, dir_name),
    )
    .with_context(|| format!("failed to write `{}`", src_dir.join("main.buff").display()))?;
    ...
    eprintln!("Initialized Buff project `{dir_name}` in {}", cwd.display());
    eprintln!("Run with: buff run src/main.buff");
    Ok(())
}
