//! Integration tests — run each v0.1 example end-to-end through `buff run`.
//!
...
///
/// `CARGO_MANIFEST_DIR` is `.../crates/buff-lang-cli`, so the workspace root is two
...
/// Run `cargo run -p buff-lang-cli -- run <example>` and return the captured stdout.
///
...
fn run_example(example: &str) -> String {
    let output = Command::new("cargo")
        .args(["run", "-q", "-p", "buff-lang-cli", "--", "run"])
...
fn test_example_ola() {
    ...
    let stdout = run_example("ola.buff");
    assert!(
        stdout.contains("Olá, Buff!"),
        "ola.buff should print `Olá, Buff!`; got:\n{stdout}"
    );
}
...
fn test_example_fibonacci() {
    ...
    // fib(10) = 55.
    let stdout = run_example("fibonacci.buff");
    assert!(
        stdout.trim().ends_with('5') && stdout.trim().contains("55"),
        "fibonacci.buff should print `55` for n=10; got:\n{stdout}"
    );
}
...
fn test_example_calculadora() {
    ...
    // add(2, 3) = 5.
    let stdout = run_example("calculadora.buff");
    let trimmed = stdout.trim();
    assert!(
        trimmed == "5",
        "calculadora.buff should print exactly `5` for add(2,3); got: `{trimmed}`"
    );
}
...
fn test_examples_directory_exists_with_v01_set() {
    ...
    for f in &["ola.buff", "fibonacci.buff", "calculadora.buff"] {
        let p = dir.join(f);
