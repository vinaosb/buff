//! Integration tests for the Deox IR dataflow graph (`deox_ast::ir`).
//!
...
fn test_add_compute_node() {
    ...
    let id = g.add_node(IrNode::compute(deox_ast::ComputeNode {
        id: NodeId(0),
...
fn test_dependency_edge() {
    let mut g = IrGraph::new();
    let a = g.add_node(IrNode::compute(deox_ast::ComputeNode {
        id: NodeId(0),
        source_expr: None,
        source_stmt: None,
        defs: Vec::new(),
    ...
    let b = g.add_node(IrNode::compute(deox_ast::ComputeNode {
        id: NodeId(0),
        source_expr: None,
        source_stmt: None,
        defs: Vec::new(),
...
fn test_finalize_entry_exit() {
    ...
    let mut g = IrGraph::new();
    let mk = |label: &str| {
        IrNode::compute(deox_ast::ComputeNode {
            id: NodeId(0),
            source_expr: None,
            source_stmt: None,
            defs: Vec::new(),
...
fn test_placeholder_node_types() {
    let mut g = IrGraph::new();
    let transfer = g.add_node(IrNode::Transfer(deox_ast::TransferNode {
        id: NodeId(0),
    ...
    let schedule = g.add_node(IrNode::Schedule(deox_ast::ScheduleNode {
        id: NodeId(0),
