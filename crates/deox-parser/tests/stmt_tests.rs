//! Integration tests for the deox-parser statement parser (T8).
//!
//! These tests exercise the parser end-to-end by feeding source strings
//! through the T6 lexer and then through [`deox_parser::stmt`] and
//! [`deox_parser::parse`] entry points.
//!
//! Coverage:
//!
...
use deox_ast::{BinaryOp, Block, Expr, FuncDecl, Ident, Literal, Param, Stmt, TypeRef};
use deox_error::{ParseError, SourceId, Span};
...
#[test]
fn test_let_string() {
    let s = parse_stmt("let nome = \"Deox\"");
    match s {
        Stmt::LetDecl { value, .. } => {
            assert!(matches!(value, Expr::Literal(Literal::String(_), _)));
            assert_eq!(shape(&value), "Lit(String(\"Deox\"))");
        }
        other => panic!("expected LetDecl, got {other:?}"),
    }
}
...
fn test_func_decl_via_parse_top_level() {
    ...
    match &decls[0] {
        deox_ast::Decl::FuncDecl(f) => assert_eq!(f.name.name, "foo"),
...
fn test_let_missing_value_errors() {
    ...
    assert_eq!(err.diagnostic.severity, deox_error::Severity::Error);
}
