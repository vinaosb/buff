...
//!
//! Each test builds a small Deox AST by hand, runs it through
//! [`deox_codegen_rust::generate_rust`], and asserts properties of the
//! resulting Rust source.

use deox_ast::common::{Block, Ident, Param};
use deox_ast::decl::FuncDecl;
use deox_ast::op::BinaryOp;
...
// ---------------------------------------------------------------------------
// 10. Full ola.deox-equivalent program — snapshot
// ---------------------------------------------------------------------------

#[test]
fn test_codegen_full_program_snapshot() {
    // func main() { print("Olá, Deox!") }
    let f = func_with_stmts(
        "main",
        vec![Stmt::ExprStmt(
            call_expr("print", vec![string_expr("Olá, Deox!")]),
            span(),
        )],
    );
    let src = generate_rust(&[f]).unwrap();
